...
SCUC_SUPERINTENDENT_TERMS <- data.frame(
  ...
  term_label = c("1976–2001", "2001–2009", "2010–2020", "2020–2024", "2024–Present"),
  start = as.Date(c("1976-01-01", "2001-01-01", "2010-01-01", "2020-02-03", "2024-02-01")),
  end = as.Date(c("2001-01-01", "2010-01-01", "2020-02-03", "2024-02-01", NA)),
  stringsAsFactors = FALSE
)

superintendent_date_layers <- function(dates) {
  dates <- as.Date(dates)
  dates <- dates[!is.na(dates)]
  if (!length(dates)) return(list())

  visible_min <- min(dates)
  ...
  if (annual_series) {
    term_start <- as.Date(paste0(format(term_start, "%Y"), "-01-01"))
    effective_end <- as.Date(paste0(format(effective_end, "%Y"), "-01-01"))
    visible_max <- as.Date(paste0(format(visible_max, "%Y"), "-12-31"))
  }

  terms$visible_start <- pmax(term_start, visible_min)
  effective_end[is.na(effective_end)] <- visible_max
  terms$visible_end <- pmin(effective_end, visible_max)
  terms <- terms[terms$visible_start <= terms$visible_end, , drop = FALSE]
  if (!nrow(terms)) return(list())
  ...
  terms$label_angle <- ifelse(
    as.numeric(terms$visible_end - terms$visible_start) < 365.25 * 2.5,
    90,
    0
  )
  ...
  list(
    ggplot2::geom_segment(
      data = terms,
      ggplot2::aes(
        x = visible_start, xend = visible_end,
        y = Inf, yend = Inf
      ),
      inherit.aes = FALSE,
      colour = "#34495E",
    ...
    ggplot2::geom_text(
      data = terms,
      ggplot2::aes(x = midpoint, y = Inf, label = label, angle = label_angle),
      inherit.aes = FALSE,
      colour = "#263746",
      size = 5,
      fontface = "bold",
...
superintendent_cohort_layers <- function(cohort_data) {
  ...
  terms <- SCUC_SUPERINTENDENT_TERMS
  terms$start_year <- as.integer(format(terms$start, "%Y"))
  terms$end_year <- as.integer(format(terms$end, "%Y"))
  terms$end_year[is.na(terms$end_year)] <- max(points$year)

  spans <- lapply(seq_len(nrow(terms)), function(i) {
    matched <- points[points$year >= terms$start_year[i] & points$year <= terms$end_year[i], , drop = FALSE]
    if (!nrow(matched)) return(NULL)
    data.frame(
      xmin = min(matched$grade) - 0.45,
      xmax = max(matched$grade) + 0.45,
      midpoint = mean(range(matched$grade)),
      label = paste0(terms$superintendent[i], "\n", terms$term_label[i]),
      label_angle = ifelse(diff(range(matched$grade)) < 2.5, 90, 0)
    )
  })
  ...
  list(
    ggplot2::geom_segment(
      data = spans,
      ggplot2::aes(x = xmin, xend = xmax, y = Inf, yend = Inf),
      inherit.aes = FALSE,
      colour = "#34495E",
      linewidth = 1.15,
      arrow = grid::arrow(
    ...
    ggplot2::geom_text(
      data = spans,
      ggplot2::aes(x = midpoint, y = Inf, label = label, angle = label_angle),
      inherit.aes = FALSE, colour = "#263746", size = 5,
      fontface = "bold", lineheight = 0.9, vjust = 1.75, check_overlap = TRUE
    )
  )
}
